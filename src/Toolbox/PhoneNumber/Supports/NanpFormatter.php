...
use Simtabi\Pheg\Toolbox\PhoneNumber\Exceptions\NanpFormatterException;
...
class NanpFormatter
{
     ...
     *
     * @return $this
     * @throws NanpFormatterException
     */
    public function parse($number, bool $wildcards = false)
    {
        // If $number is empty then return invalid
        if (empty($number)) {
            throw new NanpFormatterException('The number parameter is required');
        }

        // If the string is less than 10 characters
        if (strlen($number) < 10) {
            throw new NanpFormatterException($number.' is less than 10 characters');
        }
        ...
        if (! preg_match($characterPattern, $number)) {
            throw new NanpFormatterException($number.' contains invalid characters');
        }
        ...
        if (substr($number, 0, 1) === '+' && substr($number, 0, 2) !== '+1') {
            throw new NanpFormatterException('Only "+1" phone numbers are allowed: '.$number);
        }
        ...
        if (strlen($number) !== 12) {
            throw new NanpFormatterException('The number should be a valid 10,11 or 12 digit e164 NANP number: '.$number);
        }
        ...
        if (! preg_match($numberPattern, $number)) {
            throw new NanpFormatterException('The number needs to match the +1NXXNXXXXXX pattern: '.$number);
        }
     ...
     * @throws NanpFormatterException
     */
    public static function format(string|int $number, bool $wildcards = false): static
    {
        return (new static)->parse($number, $wildcards);
    }
