...
use Simtabi\Pheg\Toolbox\Arr\Query\Exceptions\ConditionNotAllowedException;
use Simtabi\Pheg\Toolbox\Arr\Query\Exceptions\InvalidNodeException;
use Simtabi\Pheg\Toolbox\Arr\Query\Exceptions\KeyNotPresentException;
...
abstract class QueryEngine extends Clause implements \ArrayAccess, \Iterator, \Countable
{
    ...
    {
        $this->_data = $this->convertToArray($this->_data);
    ...
    private function convertToArray($data)
    {
        ...
        foreach ($data as $key => $map) {
            if ($map instanceof QueryEngine) {
                $new_data[$key] = $this->convertToArray($map);
            } else {
                $new_data[$key] = $map;
            }
        }
