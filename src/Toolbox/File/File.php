...
use Simtabi\Pheg\Toolbox\File\Exception\PathException;
use Simtabi\Pheg\Toolbox\File\Exception\ResourceException;
use Simtabi\Pheg\Toolbox\File\Exception\TempException;
...
class File extends FileStream
{
    ...
    function permissions($path, $permissions = null)
    {
        ...
        if (null === $permissions) {
            ...
            if (false === $permissions) {
                throw new PathException('The permissions for the path "%s" could not be read.', $path);
            }
        } elseif (!chmod($path, $permissions)) {
            throw new PathException('The Unix permissions for the path "%s" could not be set to "%o".');
        }
    ...
    function resolve($link, $recursive = true)
    {
        ...
        do {
            ...
            if (false === $real) {
                throw new PathException('The symbolic link "%s" for "%s" could not be resolved.', $real, $link);
            }
     ...
     */
    function tempDir($template = null, $dir = null)
    {
        $path = $this->tempPath($template, $dir);
     ...
     */
    function tempFile($template = null, $dir = null)
    {
        $path   = $this->tempPath($template, $dir);
        $handle = fopen($path, 'x');

        if (false === $handle) {
            throw new TempException('The temporary file "%s" could not be created.', $path);
        }
    ...
    function tempPath($template = null, $dir = null)
    {
        if (null === $dir) {
            ...
            throw new TempException('The temporary directory "%s" does not exist.', $dir);
        } elseif (!is_writable($dir)) {
            throw new TempException('The temporary directory "%s" is not writable.', $dir);
        }

        if (null === $template) {
            $template = sha1(uniqid('', true));
        } elseif (false === strpos($template, '%s')) {
            throw new TempException('The temporary path template "%s" does not contain "%%s".', $template);
        } else {
