...
final class Xml2Array
{
    ...
    {
        // default value:
        $config = empty($config) ? XmlToArrayConfig::DEFAULTS : $config;
        $init   = new self($config);
    ...
    private function extractArray(): array
    {
        ...
        $convertedElement = $this->xml2Array($this->xml->documentElement);
     ...
     */
    private function xml2Array(DOMNode $node)
    {
        $output = [];
    ...
    private function collectAttributes(DOMNode $node, $output)
    {
        ...
        // if it's a leaf node, store the value in @value instead of directly it.
        if (!is_array($output)) {
