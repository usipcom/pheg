...
class Interpreter {
    ...
    public function __construct()
    {
        ...
        for($i = 0; $i < count(self::COLOR_NAME_MAP); $i++)
        {
            $color = "#" . self::COLOR_NAME_MAP[$i][0];
            $rgb   = $this->rgb($color);
            $hsl   = $this->hsl($color);

            array_push(self::COLOR_NAME_MAP[$i], $rgb[0], $rgb[1], $rgb[2], $hsl[0], $hsl[1], $hsl[2]);
        }
    }
    ...
    public function name($color){
        ...
        for($i = 0; $i < count(self::COLOR_NAME_MAP); $i++)
        {
            if($color == "#" . self::COLOR_NAME_MAP[$i][0]){
                return [
                    "hex"   => "#" . self::COLOR_NAME_MAP[$i][0],
                    "name"  => self::COLOR_NAME_MAP[$i][1],
                    "exact" => true,
                ];
            }

            $ndf1 = pow($r - self::COLOR_NAME_MAP[$i][2], 2) + pow($g - self::COLOR_NAME_MAP[$i][3], 2) + pow($b - self::COLOR_NAME_MAP[$i][4], 2);
            $ndf2 = pow($h - self::COLOR_NAME_MAP[$i][5], 2) + pow($s - self::COLOR_NAME_MAP[$i][6], 2) + pow($l - self::COLOR_NAME_MAP[$i][7], 2);
            $ndf = $ndf1 + $ndf2 * 2;
        ...
        if($cl < 0){
            ...
            return [
                "hex"   => "#" . self::COLOR_NAME_MAP[$cl][0],
                "name"  => self::COLOR_NAME_MAP[$cl][1],
                "exact" => false,
            ];
        }
    }
